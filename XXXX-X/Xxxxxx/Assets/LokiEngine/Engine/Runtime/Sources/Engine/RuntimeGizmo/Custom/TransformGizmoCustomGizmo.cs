...
{
	/// <summary>
	/// Currently does not really handle TransformType.All
	/// </summary>
	public class TransformGizmoCustomGizmo : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			mMask = LayerMask.GetMask(LayerMask.LayerToName(gizmoLayer));
		...
		private void CheckForSelectedAxis()
		{
			...
			{
				RaycastHit hitInfo;
				if (Physics.Raycast(transformGizmo.viewer.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, mMask))
				{
